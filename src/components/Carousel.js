                ...
                {resultApi.map((api, key) => ( // on map le tableau avec les données de notre api SI)
                    <div api={api} key={key} >
                        <div className="card-carousel"><h1>{api.fields.title}</h1>
                            {api.fields.date_description && <h5><span>📅 </span><u>DATE</u> :<br /> {api.fields.date_description.split("<br />").join(" ")}</h5>}<br />
                            {api.fields.address_name && <h5><span>📍 </span><u>ADRESSE</u> :<br /> {api.fields.address_name} - {api.fields.address_street}, {api.fields.address_zipcode} {api.fields.address_city}</h5>}<br />
                            {api.fields.price_detail &&  <h5><span>💶 </span><u>PRIX</u> :<br />{api.fields.price_detail}</h5>}<br />
                            {api.fields.access_link && <p><a className="link" target="_blank" rel="noreferrer" href={api.fields.access_link}>Voir le site de l'évenement</a></p>}
                        </div>
